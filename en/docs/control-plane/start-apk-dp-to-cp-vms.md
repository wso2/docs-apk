...
!!! Note
    You can change the default hostnames via values.yaml. For example, to use dev.gw.example.com for APIs and dev.apk.example.com for system APIs:
    - wso2.kgw.listener.hostname: 'dev.apk.example.com'
    - wso2.kgw.dp.gateway.listener.hostname: 'dev.gw.example.com'
...
3. Change the configurations in the ```values.yaml``` file in the Kubernetes Gateway Agent Helm Chart, refer to the [Advanced Configuration for agent](../../../control-plane/apim-apk-agent-deploy) for more information.
