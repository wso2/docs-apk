...
To configure Adapter logs, update the `values.yaml`'s `wso2.apk.dp.adapter.logging` section with the following values. Instructions in the <a href="../../setup/Customize-Configurations" target="_blank">customize configurations</a> section will guide you through the process of acquiring the `values.yaml` file.
