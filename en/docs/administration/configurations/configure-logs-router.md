...
You can enable Router access logs by using the following configs. Router access logs related configurations are in the `log_config.toml` file, which is located in ConfigMap file located in the following file in the APK helm chart.
...
You can do this by setting the `TRAILING_ARGS` environment variable in the Router. For example, Add the following line under router configuration in the gateway-runtime-deployment.yaml in the directory `apk/helm-charts/templates/data-plane/gateway-components/gateway-runtime/`.
...
To enable the wire logs, add the following configuration to the `log_config.toml` file, which is located in ConfigMap file located in the following file in the APK helm chart.
